...
def book_list(request):
    ...
    return render(request, "book_list.html", {"page_obj": page_obj, "authors": authors, "categories": categories})
...
def search_books(request):
    ...
    results = []
    if query:
        results = Book.objects.filter(Q(title__icontains=query) |Q(author__first_name__icontains=query) |Q(author__last_name__icontains=query) |Q(code__icontains=query) |Q(category__name__icontains=query)).distinct()
